...
export default function ClassroomDetail() {
  ...
  // Group photo upload
  const onDropGroupPhoto = async (acceptedFiles: File[]) => {
  ...
  // Student assignment
  const handleStudentAssignment = (faceId: string, rollNumber: string, studentName: string) => {
  ...
  const submitStudentAssignments = async () => {
    const assignments = Object.entries(studentAssignments).map(([faceId, data]: [string, any]) => ({
      face_id: faceId,
  ...
  // Attendance taking
  const onDropAttendance = async (acceptedFiles: File[]) => {
    if (acceptedFiles.length === 0) return;
      ...
      >
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 max-h-96 overflow-y-auto">
            {facesForLabeling.map((face: any, index) => (
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {attendanceResults.session.attendanceResults.map((result: any, index: number) => (
